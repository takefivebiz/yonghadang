...
최근 변화는 네 가지다. 첫째, analyze page가 real Claude API를 실제로 호출하고 `result_scenes`에 저장한다. 둘째, result page가 localStorage를 우선 사용하되 DB fallback/cache reuse를 시작했다. 셋째, OAuth 로그인과 마이페이지의 실제 `analysis_sessions` 조회가 연결됐다. 넷째, result page Scene 01을 “감정 구조 추적 리포트” 방향으로 재정리하고, 감정 신호 시각화 시스템의 첫 template을 구현했다.
...
- emotional signal visualization: Scene role → archetype → template → signal data 구조 도입
...
- 진행중: result page redesign — Scene 01 receipt/report 구조, psychological case file 톤, 감정 신호 시각화 시스템 기반 구축 완료
...
## 감정 신호 시각화 시스템 기반 구축 (2026-05-20)

이번 작업은 단순 UI 추가가 아니라, VEIL 결과 리포트를 “감정 구조 시각화 시스템” 방향으로 확장한 작업이다. 결과 리포트는 AI 상담 답변이 아니라, 사용자의 감정 사건 파일 안에서 감정 간 긴장 구조와 반복 신호를 추적하는 기록물로 정리한다.

### 시스템 방향

VEIL은 감정을 설명하는 것이 아니라, 감정 간 긴장 구조와 반복 신호를 추적한다.

감정 신호 시각화는 일반 analytics chart가 아니라, 사용자의 입력에서 추출된 감정 구조를 “기록물 안의 증거 조각”처럼 보여주는 시스템이다.
...
- `src/lib/types/scene-signal.ts`
  - `SignalTemplateType` 추가
  - `EmotionalArchetype` 추가
  - `SceneSignal` 추가
  - `SceneVisualRole` 추가
- `src/components/result/signals/DonutSignalTemplate.tsx`
  - donut 기반 imbalance visualization template 분리
  - `SceneSignal[]`를 받아 data-driven으로 렌더링
- `src/components/result/signals/SignalVisualization.tsx`
  - signal renderer 추가
  - 현재는 `template === "donut"`만 렌더링
  - 이후 pulse/timeline, balance meter, threshold gauge 등을 붙일 수 있는 구조
- `src/components/result/scene-content.tsx`
  - Scene 01 하드코딩 도넛 UI 제거
  - `scene01Signals: SceneSignal[]`로 mock signal data 구조화
  - Scene 01에서 `SignalVisualization`을 통해 렌더링

### Scene 01 현재 상태

- receipt compact화 완료
- box-in-box 구조 감소
- Scene 01 report section을 별도 document section으로 분리
- `핵심 관찰` → `핵심 징후`로 변경
- signal section label을 `강하게 관찰된 감정 신호`로 변경
- donut 기반 emotional imbalance UI 적용
- AI 상담체를 줄이고 psychological report / quiet archive 방향 강화

현재 Scene 01 archetype:
...
- dot signal 방식은 보류
- abstract signal rhythm 방식은 보류
- pair imbalance visualization을 채택
- progress/dashboard 문법은 일부 허용하되, 숫자/퍼센트는 화면에 노출하지 않음
- 전체 tone은 weird archive / psychological case file 유지
- signal은 punch를 반복하지 않고, punch를 만든 감정 압력 구조를 보여줘야 함

### 다음 예정 작업

- Scene 02 signal visualization 설계
- pulse/timeline grammar 실험
- Scene 03 split comparison / balance meter 설계
- generate-result.ts signal generation 구조 설계
- archetype extraction logic 검토
- signal-template system 확장
...
- [ x ] 감정 신호 시각화 시스템 1차 기반 (`SceneSignal` 타입 / signal renderer / donut template / Scene 01 적용)
...
- Result page redesign — Scene 01 이후 Scene 02~06 signal grammar 확장
