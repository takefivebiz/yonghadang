...
- **미니 히어로 섹션**
  - **H1 (한 줄 문구)**: "흐릿했던 나의 이야기가, 선명해지는 순간"
    - 기획 의도: "흐릿했던"으로 자기 이해 부족에 공감 → "선명해지는"으로 명확한 인사이트 약속 → "순간"으로 즉시성·기대감 유발
    - 그래디언트 강조: "선명해지는 순간" 구간에 브랜드 그래디언트 적용
  - **부제 (상세 소개)**:
    - 1행: "마음이 흔들릴 때, 선택 앞에서 멈출 때"
    - 2행: "사주 · MBTI · 타로 · 점성술, 당신만의 이야기를 꺼내드립니다"
    - 기획 의도: 연애/선택/고민 상황에 직접 공감 → 4가지 분석 도구를 기능이 아닌 "이야기를 꺼내는 수단"으로 포지셔닝
  - **신뢰 지표** (순서 중요 — 앞에 올수록 클릭 장벽 제거 효과 큼):
    - "✦ 무료로 시작 · ✦ 3분 안에 완성 · ✦ 수천 명이 선택한 리포트"
    - 기획 의도: "무료로 시작"을 맨 앞에 배치해 진입 장벽 최소화. "수천 명"은 사회적 증명(론칭 초기에는 플레이스홀더, 실제 수치로 교체)

- **콘텐츠 탐색 섹션**
  - **섹션 헤더**: "지금 당신에게 필요한 건 무엇인가요?"
  - **섹션 부제**: "4가지 방법으로, 당신만의 이야기를 꺼내드립니다"
    - 기획 의도: "시작하세요"(일방적 지시)에서 "필요한 건 무엇인가요?"(유저 중심 질문)로 전환해 자발적 탐색 유도
  - **카테고리 탭 필터** (탭 선택 시 탭 하단에 카테고리 설명 fade-in 노출):
    - 전체: "당신에게 맞는 분석 방법을 찾아보세요"
    - MBTI: "내 성격의 뿌리, 관계 패턴, 직업 적합성까지"
    - 사주: "태어난 순간이 담고 있는 나만의 운명 코드"
    - 타로: "지금 이 순간, 가장 필요한 메시지를 카드가 전합니다"
    - 점성술: "별의 위치가 말해주는 당신의 본질과 가능성"
  - **콘텐츠 카드 그리드**
    - 카드 구성: 썸네일, 제목, 후킹 한 줄 설명, 카테고리 태그, 가격 표시
    - 가격 표시: "무료 체험 후 ₩X,XXX" 형태로 무료 진입점 강조
    - 인기/신규 배지 표시 (🔥 지금 인기 / ✨ 신규)
    - 클릭 시: `/start` 이동 + 해당 콘텐츠 preselect
  - **콘텐츠별 카드 후킹 문구** (감정 공감 + 궁금증 유발 구조):
    | 콘텐츠 | 후킹 설명 문구 |
    | ------ | ------------- |
    | MBTI 기본 유형 해석 | 나조차 몰랐던 내 성격의 뿌리를 꺼내드립니다 |
    | 심층 사주 분석 | 언제 기회가 오는지, 어떤 사람이 맞는지 — 사주가 답합니다 |
    | 오늘의 타로 리딩 | 오늘 하루를 위한 메시지, 지금 카드를 뽑아보세요 |
    | 별자리 출생 차트 | 내가 어떤 사람으로 설계됐는지, 별이 알고 있습니다 |
    | MBTI 궁합 분석 | 우리가 자꾸 부딪히는 이유, 성격에 있었습니다 |
    | 신년 운세 사주 | 올해 언제가 내 인생의 터닝포인트일까요 |
    | 연애 타로 리딩 | 그 사람 마음, 타로 카드가 솔직하게 말해줍니다 |
    | 별자리 궁합 | 이 관계, 우주가 설계한 건지 — 별자리가 증명합니다 |
...
| --- | ------------------ | -------------------- | ----------------------------------------------------------- | ------------------------------------------------------------------------- | --------- | ----------------------------------- |
| 1   | 메인 랜딩페이지    | `/`                  | `is_active = false`인 콘텐츠가 전부일 때 카드 그리드 렌더링 | 카드 0개 + 빈 상태 UI 표시, 에러 없음                                     | [x]       | 빈 상태 UI(`🌙 준비 중인 콘텐츠입니다.`) 코드상 구현 확인(`length===0` 분기). 단, `is_active` 필드가 `Content` 타입 및 더미데이터에 없어 필터링 로직 미구현 — 백엔드 연동 후 재테스트 필요 |
| 2   | 메인 랜딩페이지    | `/`                  | 카테고리 탭 전환 시 URL 쿼리 파라미터와 카드 목록 동기화    | 탭 선택 시 해당 category 카드만 필터링, 브라우저 뒤로가기 시 이전 탭 복원 | [x]       | `useSearchParams` + `router.push(?category=)` 구현 완료. `"all"` 선택 시 파라미터 제거. 유효하지 않은 값 폴백 처리. `Suspense` 경계 추가 |
| 3   | 메인 랜딩페이지    | `/`                  | 네트워크 오류로 콘텐츠 목록 로드 실패                       | 에러 메시지 또는 재시도 UI 표시, 화면 크래시 없음                         | [ ]       | 현재 더미데이터 단계로 fetch 자체가 없어 테스트 불가. 에러 상태 UI 미구현 — 백엔드 연동 시 `error.tsx` 또는 try/catch 에러 fallback 구현 필요 |
| 4   | 콘텐츠 상세 페이지 | `/mbti/basic` 등     | 존재하지 않는 slug(`/mbti/ghost`)로 직접 접근               | Next.js 404 페이지 반환                                                   | [x]       | mbti/saju/tarot/astrology 4개 카테고리 `[slug]/page.tsx` 모두 `if (!detail) notFound()` 구현 확인. 더미데이터에 없는 slug 접근 시 Next.js 기본 404 반환 정상 동작 |
| 5   | 콘텐츠 상세 페이지 | `/mbti/basic` 등     | 필수 입력 필드를 비운 채 결제하기 버튼 클릭                 | 유효성 검사 에러 메시지 표시, 결제 페이지 이동 불가                       | [x]       | 실제 폼은 `/start` 페이지에 위치. `hasAttempted` 상태 추가 — 제출 시도 시 각 필드 하단에 인라인 에러 메시지 표시(생년월일 미입력, 성별 미선택, 타로 질문 미입력). 버튼을 항상 클릭 가능하게 변경 후 `isValid() === false`이면 에러 표시 후 이동 차단 |
| 6   | 콘텐츠 상세 페이지 | `/mbti/basic` 등     | `is_active = false` 콘텐츠 URL 직접 접근                    | 404 또는 서비스 종료 안내 페이지 반환                                     | [x]       | `Content` 타입에 `is_active?: boolean` 추가. 더미데이터 전체 `is_active: true` 설정. mbti/saju/tarot/astrology 4개 `[slug]/page.tsx` 모두 `detail.is_active === false` 시 `notFound()` 처리 구현. 백엔드 연동 후 DB 값으로 자동 동작 |
| 7   | 결제 페이지        | `/payments`          | 비회원 전화번호 입력 없이 결제 위젯 진행 시도               | 전화번호 필수 입력 에러 표시, 위젯 진행 차단                              | [x]       | 필드별 에러 분리 구현: 빈 필드 "입력해주세요" / 형식 오류 "올바른 형식으로 입력해주세요" 메시지 분리. 에러 발생 시 해당 input 빨간 테두리 하이라이트 적용. `aria-invalid` + `aria-describedby` 접근성 속성 추가. 입력 수정 시 해당 필드 에러 즉시 초기화. 위젯 차단 정상 동작 확인 |
| 8   | 결제 페이지        | `/payments`          | 결제 진행 중 브라우저 새로고침 후 재접근                    | 입력 정보 유지 또는 초기화 안내, 중복 주문 미생성                         | [x]       | sessionStorage에 저장된 `pendingInput.orderId` 가 있으면 `orderIdRef.current` 에 복원하여 동일 orderId 재사용(중복 주문 방지). 복원 시 "이전 입력 정보가 복원되었습니다" 안내 배너(`role="status"`) 자동 표시. 전화번호·비밀번호 입력 정보도 함께 복원됨. 30분 만료 시 sessionStorage 자동 제거(기존 동작 유지) |
