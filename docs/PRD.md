...
| H-04 | CategoryTabs 렌더링                   | 정상 | 연애/인간관계/직업·진로/감정 탭 노출        |
...
| #    | 테스트 항목                               | 유형 | 기대 결과                                    |
| ---- | ----------------------------------------- | ---- | -------------------------------------------- |
| R-18 | 개별 Scene 구매 모달 (900원) 렌더링       | 정상 | 가격, 제목 정확히 표시                       |
| R-19 | 전체 구매 모달 (2,900원) 렌더링           | 정상 | 가격, 내용 정확히 표시                       |
...
| E2E-02 | **신규 비회원 전체 구매 플로우** : 홈 → 콘텐츠 진입 → 입력 → 결과 → 전체 구매(2,900원) → 전체 열람                              | 정상 | 전체 Scene 잠금 해제                   |
...
| E2E-07 | **개별 구매 후 전체 구매 차액 플로우** : 개별 Scene 900원 결제 → 전체 구매 선택 → 2,000원 청구                                  | 엣지 | 기구매 금액 제외한 차액만 청구         |
...
| E2E-10 | **모바일 전체 플로우** : 모바일 뷰포트에서 홈 → 콘텐츠 → 분석 → 결과 → 결제                                                     | 엣지 | 모바일 레이아웃 정상, 터치 이벤트 정상 |
