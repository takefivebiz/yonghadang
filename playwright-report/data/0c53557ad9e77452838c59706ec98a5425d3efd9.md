...
- Name: analyze-page.spec.ts >> A: 분석 입력 페이지 (/analyze/[session_id]) >> A-21: 모든 보정 질문 답변 완료 후 완료 화면 표시
- Location: tests/e2e/analyze-page.spec.ts:330:3
...
Locator: locator('text=/모든 질문이 끝났어|흐름을 정리하고 있어/').first()
Expected: visible
Timeout: 3000ms
...
Call log:
  ...
  - waiting for locator('text=/모든 질문이 끝났어|흐름을 정리하고 있어/').first()
