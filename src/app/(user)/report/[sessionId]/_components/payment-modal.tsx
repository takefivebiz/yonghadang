...
import { handlePaymentSuccess } from '@/lib/actions/order';
...
export const PaymentModal = ({ pendingOrder, onClose, onSuccess }: PaymentModalProps) => {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        console.error('[PaymentModal] 인증 확인 실패:', error);
        setUser(null);
      }
    };
      ...
      try {
        ...
        if (!widgetRef.current) {
          ...
          const widgets = tossPayments.widgets({
            customerKey: user ? `member_${Date.now()}` : `guest_${Date.now()}`,
          });
  ...
  }, [user, pendingOrder.paidQuestionIds.length, pendingOrder.isFullBundle]);
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      // TODO: [백엔드 연동] 결제 후 order 저장은 handlePaymentSuccess에서 처리
      // grantGuestAccess로 30분 토큰 발급했으므로, 결제 직후 본인확인 불필요
      // 토큰 만료 후 재진입 시에만 본인확인 요구

      // 먼저 백엔드: Supabase 주문 생성 + 유료 리포트 생성 (리다이렉트 전에 완료)
      console.log('[PAYMENT MODAL] calling handlePaymentSuccess', {
        sessionId: pendingOrder.sessionId,
        orderId,
        questionIds: pendingOrder.paidQuestionIds,
        isBundle: pendingOrder.isFullBundle,
      });
      try {
        await handlePaymentSuccess({
          sessionId: pendingOrder.sessionId,
          questionIds: pendingOrder.paidQuestionIds,
          isBundle: pendingOrder.isFullBundle ?? false,
          phoneNumber: !user ? guestInfo.phoneNumber.replace(/[^0-9]/g, "") : undefined,
          orderId,
        });
        console.log('[PAYMENT MODAL] handlePaymentSuccess completed successfully');
      } catch (err) {
        console.error("[PAYMENT MODAL] handlePaymentSuccess failed:", err);
      ...
      // 토스페이먼츠 결제 요청 (order 생성 후)
      await widgetRef.current.requestPayment({
        orderId,
        orderName: `${pendingOrder.category} 리포트 ${pendingOrder.paidQuestionIds.length}개 질문`,
        customerName: user ? 'Member' : '비회원',
        customerEmail: user ? 'member@example.com' : guestInfo.phoneNumber,
        customerMobilePhone: !user ? guestInfo.phoneNumber.replace(/\D/g, '') : undefined,
        successUrl,
        failUrl,
      });

      // requestPayment가 redirect 없이 resolve된 경우 (로컬 환경)
      onSuccess(orderId);
    } catch (error) {
