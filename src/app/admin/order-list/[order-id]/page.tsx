...
const InfoRow = ({ label, value }: { label: string; value?: string }) =>
  value ? (
    <div className="flex flex-col gap-0.5 sm:flex-row sm:gap-4">
      <span className="w-32 shrink-0 text-sm font-medium text-foreground/50">{label}</span>
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-foreground">{order.category} 분석</h1>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        {/* 유저 응답 데이터 */}
        <div className="flex flex-col gap-4 rounded-2xl bg-card p-6 shadow-sm">
          <h2 className="text-base font-semibold text-foreground">응답 데이터</h2>
          <div className="flex flex-col gap-3">
        ...
        {/* AI 리포트 */}
        <div className="flex flex-col gap-4 rounded-2xl bg-card p-6 shadow-sm">
          <h2 className="text-base font-semibold text-foreground">무료 리포트 요약</h2>
          {order.freeReportText ? (
            <p className="text-sm leading-relaxed text-foreground/80">{order.freeReportText}</p>
          ) : (
