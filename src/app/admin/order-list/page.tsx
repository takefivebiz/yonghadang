...
const AdminOrderListPage = () => {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* 필터 섹션 */}
      <div className="rounded-2xl p-4 md:p-6 gap-4 flex flex-col" style={{ background: 'linear-gradient(135deg, rgba(100, 149, 237, 0.1), rgba(75, 0, 130, 0.08))', border: '1px solid rgba(230, 230, 250, 0.15)' }}>
        {/* 검색창 */}
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-lg border py-2.5 pl-9 pr-4 text-sm outline-none transition-all focus:border-blue-400 focus:shadow-lg"
            style={{
        ...
        <div className="grid grid-cols-2 md:grid-cols-2 gap-3">
          <select
            ...
            className="rounded-lg border px-3 py-2.5 text-sm outline-none transition-all focus:border-blue-400 focus:shadow-lg"
            style={{
              background: 'rgba(255, 255, 255, 0.05)',
              borderColor: 'rgba(230, 230, 250, 0.2)',
              color: '#F5F5F5',
      ...
      {/* 데이터 테이블 - 데스크톱 */}
      <div className="hidden lg:block overflow-hidden rounded-2xl shadow-sm" style={{ background: 'linear-gradient(135deg, rgba(100, 149, 237, 0.1), rgba(75, 0, 130, 0.08))', border: '1px solid rgba(230, 230, 250, 0.15)' }}>
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map((order) => (
            ...
            >
              ...
              <span className="whitespace-nowrap text-xs" style={{ color: '#B8A8D8' }}>
                {formatDate(order.createdAt)}
              </span>
              <span style={{ color: '#D4C5E2' }}>{order.nickname}</span>
              ...
              <span className="whitespace-nowrap font-semibold" style={{ color: '#F0E6FA' }}>
      ...
      <div className="lg:hidden flex flex-col gap-3">
        {filtered.length === 0 ? (
          <div className="rounded-2xl p-8 text-center" style={{ background: 'linear-gradient(135deg, rgba(100, 149, 237, 0.1), rgba(75, 0, 130, 0.08))', border: '1px solid rgba(230, 230, 250, 0.15)' }}>
            <p className="text-sm" style={{ color: '#B8A8D8' }}>검색 결과가 없습니다.</p>
          </div>
        ) : (
          filtered.map((order) => (
            <Link
              ...
              style={{
                background: 'linear-gradient(135deg, rgba(100, 149, 237, 0.08), rgba(75, 0, 130, 0.05))',
            ...
            >
              ...
              <div className="flex items-start justify-between mb-3">
                <div>
                  <p className="font-semibold text-sm" style={{ color: '#F0E6FA' }}>
                    {order.category} 분석
                  </p>
                  <p className="text-xs mt-1 font-mono" style={{ color: '#B8A8D8' }}>
                    {order.id}
                  </p>
                </div>
                <StatusBadge status={order.status} />
              ...
              <div className="flex items-center gap-2 mb-3 flex-wrap">
                ...
                <span className="text-xs font-semibold" style={{ color: '#F0E6FA' }}>
                  {order.amount.toLocaleString()}원
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                <div>
                  <p style={{ color: '#B8A8D8' }} className="text-xs mb-0.5">닉네임</p>
                  <p style={{ color: '#D4C5E2' }} className="truncate font-medium">
                    {order.nickname}
                  </p>
                </div>
                <div>
                  <p style={{ color: '#B8A8D8' }} className="text-xs mb-0.5">주문 일시</p>
                  <p style={{ color: '#D4C5E2' }} className="text-xs">
                    {formatDate(order.createdAt)}
                  </p>
                </div>
              </div>
