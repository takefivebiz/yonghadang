...
### 메시지 구성 & Punch 위치 규칙
구조: ai (opener) → [내용 전개] → punch (중간~후반) → [마무리]
...
**권장 구조:**
1. ai (opener: 흐름 이어받기) → ai (상황 설명) → punch (핵심) → ai (마무리)
2. ai (opener) → punch (구조 드러냄) → ai (관점 전환) → ai (선택 제시)
3. ai (opener) → ai (감정 누적) → punch (반전/깨달음) → memo

### 메시지 타입별 역할
- **ai (opener)**: 무료 scene을 이어받으며 현재 scene으로 진입 (2~3문장)
- **ai (전개)**: 감정 누적, 구조 설명, 관점 이동 (2~3문장)
- **punch**: "읽다가 갑자기 찔리는 한 줄" (필요시 \\n으로 리듬)
  - Scene 중간~후반부에만
  - 1~2줄, 임팩트 있게
  - 예: "확인하고 싶은데,\\n놓지 못하고 있는 거야."
- **memo**: 40~60자 요약 (선택사항)
...
### 무료 vs 유료 구분
무료: "짧고 강하게" (한 문장도 강렬하게)
유료: "깊고 이어지게" (흐름 따라가다가 깊어지기)`;
};
