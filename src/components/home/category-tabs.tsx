    ...
    <div className="flex justify-center gap-2.5 overflow-x-auto pb-1">
      {CATEGORIES.map((category) => (
        <a
          key={category}
          href={`#${category}`}
          className={`shrink-0 rounded-full border px-3.5 py-1.5 text-xs font-medium tracking-wide transition-all duration-150 sm:px-5 sm:py-2 sm:text-sm ${CATEGORY_TAB_STYLE[category]}`}
        >
