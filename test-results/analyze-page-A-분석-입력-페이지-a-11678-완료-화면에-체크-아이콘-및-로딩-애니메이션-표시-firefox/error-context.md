...
- Name: analyze-page.spec.ts >> A: 분석 입력 페이지 (/analyze/[session_id]) >> A-22: 완료 화면에 체크 아이콘 및 로딩 애니메이션 표시
- Location: tests/e2e/analyze-page.spec.ts:375:3
...
Locator: locator('svg path[d*="5 13"]').first()
Expected: visible
Timeout: 3000ms
...
Call log:
  ...
  - waiting for locator('svg path[d*="5 13"]').first()
