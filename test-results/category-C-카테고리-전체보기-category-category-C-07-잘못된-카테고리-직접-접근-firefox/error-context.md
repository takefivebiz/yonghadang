...
- Name: category.spec.ts >> C: 카테고리 전체보기 (`/category/[category]`) >> C-07: 잘못된 카테고리 직접 접근
- Location: tests/e2e/category.spec.ts:70:3
...
Call log:
  - navigating to "http://localhost:3000/category/invalid-category-xyz-9999", waiting until "load"
...
> 71  |     await page.goto('/category/invalid-category-xyz-9999');
  ...
  115 |     
  116 |     const grid = page.locator('[data-testid="category-grid"]');
  117 |     if (await grid.isVisible()) {
  118 |       const gridTemplateColumns = await grid.evaluate(el => 
  119 |         window.getComputedStyle(el).gridTemplateColumns
  120 |       );
  121 |       
  122 |       // 1열: "1fr" 또는 단일 값
  123 |       const columns = gridTemplateColumns.split(' ').filter(s => s.trim().length > 0);
  124 |       // 모바일에서는 1열이어야 함
  125 |       expect(columns.length).toBeLessThanOrEqual(1);
  126 |     }
  127 |   });
  128 | });
  129 | 
