...
- Name: global-layout.spec.ts >> G: 글로벌 레이아웃 / 네비게이션 >> G-11 ~ G-15: 모달 상호작용 및 에지 케이스 >> G-15: 존재하지 않는 경로 직접 접근
- Location: tests/e2e/global-layout.spec.ts:151:5
...
Call log:
  - navigating to "http://localhost:3000/invalid-path-that-does-not-exist-12345", waiting until "load"
...
```ts
  ...
  142 | 
  143 |       // 모달 열림 확인
  144 |       await expect(page.locator('h3', { hasText: /자주 묻는 질문/ })).toBeVisible();
  145 | 
  146 |       // body.style.overflow = 'hidden' 으로 스크롤이 차단됐는지 직접 확인
  147 |       const bodyOverflow = await page.evaluate(() => document.body.style.overflow);
  148 |       expect(bodyOverflow).toBe('hidden');
  149 |     });
  150 | 
  151 |     test('G-15: 존재하지 않는 경로 직접 접근', async ({ page }) => {
> 152 |       await page.goto('/invalid-path-that-does-not-exist-12345');
      |                  ^ Error: page.goto: Could not connect to the server.
  153 |       
  154 |       // 404 또는 홈으로 리다이렉트
  155 |       const url = page.url();
  156 |       expect(url).toMatch(/localhost:3000\/?$|404|invalid/);
  157 |     });
  158 |   });
  159 | });
  160 | 
